...
double Parser::parse(std::vector<Token> tokens)
{
	...
	std::map<std::string, int> precedence{{"+", 1}, {"-", 1}, {"%", 1}, {"^", 3}, {"*", 2}, {"/", 2}};
	...
	for (const auto &token : tokens)
	{
		...
		{
			while (!operators.empty() && operators.top().type == TokenType::OPERATOR && (precedence[operators.top().value] >= precedence[token.value]))
			{
